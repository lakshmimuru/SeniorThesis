...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, inp_feature, num_blocks, max_len, d_model, num_heads, dropout=0.):
        super().__init__()
        
        self.feature_encoder = linear(inp_feature,d_model)
        if num_blocks > 0:
        ...
        self.layer_norm = nn.LayerNorm(d_model)
        self.positional_encoder = PositionalEncoding(max_len, d_model)
        self.proj_out = linear(d_model, inp_feature)
    ...
    def forward(self, input):
        ...
        input = self.positional_encoder(self.feature_encoder(input))
        for block in self.blocks:
            input = block(input,None)
    
        return self.proj_out(self.layer_norm(input))
    
    
    def generate(self, pre_int_seq, time_range):
        
        seq = pre_int_seq
        self.eval()
        
        while seq.shape[1] < time_range:
            
            with torch.no_grad():
                out = self.forward(seq[:,-self.max_len:])
                pred = out[:,-1]
            seq = torch.cat((seq,pred.unsqueeze(1)),1)
        
        self.train()
        
        return seq[:,:time_range]
